if(room == rmMain){
	...
	#region | Pause Menu
		if(global.Pause){ 
			...
			if(room == rmJoinLobby){
				var _nH = oManager_network.number_of_hosts;
									
				var y_buffer = 32, x_buffer = 16;
				var start_y = (global.CameraHeight/2) - ((((_nH-1)/2) * y_buffer)), start_x = global.CameraWidth/2;
				var _c = c_black
				var _f = fntDebug;
				var scale =  0.5;
				...
				var yy = 0; repeat(_nH){
					...
					if(yy == network_cursor){
						_c = c_white;
						xo = + (x_buffer/2);
					}
		
					draw_text_transformed_color(ltx-xo, lty, "Host: "+ string(yy) ,scale, scale, 0, _c , _c, _c, _c, 1);
					yy++;
				}
